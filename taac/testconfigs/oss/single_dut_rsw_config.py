...
def _load_constants():
    ...
    if not path:
        ...
        module, source = rsw_constants, "in-repo rsw_constants.py"
    else:
        ...
        source = f"{path!r} ({_CONSTANTS_PATH_ENV})"
    # Validate BOTH branches: if the in-repo module ever drops or renames one
    # of these, failing here beats a bare AttributeError mid-construction.
    missing = [n for n in _REQUIRED_CONSTANTS if not hasattr(module, n)]
    if missing:
        raise ValueError(
            f"Constants module {source} is missing required names: {missing}. "
            f"See taac/testconfigs/single_dut_rsw/rsw_constants.py for the "
...
TEST_CONFIG = gen_rsw_test_config(
    ...
    local_mac_address=os.environ.get("TAAC_DUT_MAC") or rsw.RSW_LOCAL_MAC_ADDRESS,
