...
The Playbook runs the pre-deployed ``setup_portchannel_subinterfaces.sh`` on
each box as ordered steps under FastCli (far faster than streaming ``configure``
blocks over FCR). TestConfig teardown runs the companion
``cleanup_portchannel_subinterfaces.sh`` so cleanup remains lifecycle-guaranteed.
...
The ordered validation step uses ``OpenrSparkNeighborHealthCheck`` with
...
DEFAULT_BASELINE_NEIGHBOR_COUNT: int = 1
...
# Validation retry envelope so transient convergence lag does not FAIL the run.
...
def create_openr_portchannel_subif_test_config(
    ...
    baseline_neighbor_count: int = DEFAULT_BASELINE_NEIGHBOR_COUNT,
    ...
    Args:
        ...
        baseline_neighbor_count: Exact Open/R adjacency count required before setup.
        ...
        postcheck_retry_count: Retries for exact adjacency condition polling.
        postcheck_retry_delay_seconds: Base delay between validation retries.
    ...
    return TestConfig(
        ...
        playbooks=[
            create_openr_subif_adjacency_scale_playbook(
                ...
                num_subinterfaces=num_subinterfaces,
                start_vlan=start_vlan,
                dut_octet=DUT_OCTET,
                peer_octet=PEER_OCTET,
                baseline_neighbor_count=baseline_neighbor_count,
