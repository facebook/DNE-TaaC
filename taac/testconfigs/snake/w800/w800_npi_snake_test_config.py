...
Single-DUT loopback qualification for the W800 NPI program, covering both
400G testbeds in ash6: WEDGE800CACT (Cisco/Leaba GRAPHENE202X) and
WEDGE800BACT (Broadcom).

Both are cabled as ONE continuous serpentine rather than the per-subport
chains the Minipack3/Kodiak3 snakes use: ``eth1/N/5`` is jumpered to
``eth1/(N+1)/1`` and each OSFP cage forwards ``/1`` to ``/5`` internally,
so a single chain walks all 64 ports (32 cages x 2 subports, VLANs
2000-2031) between the two IXIA ports. That ordering is fixed by the
landed static topologies ``static_topologies/fboss338726358_ash6.cconf``
and ``static_topologies/fboss338826479_ash6.cconf``, and it is why each
config here declares one ``SnakeConfig`` instead of two.

The two beds differ only in silicon and hostname; the snake shape, the
playbook scope, and the IXIA wiring model are identical.
...
WEDGE800BACT_NPI_SNAKE_TEST_CONFIG_400G = gen_snake_test_config(
