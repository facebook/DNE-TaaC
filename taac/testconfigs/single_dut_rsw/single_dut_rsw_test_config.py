...
L3 addressing and 4 BGP peer entries per port (see the parameters listed
below); the IXIA side is brought up by the
...
from taac.playbooks.playbook_definitions import (
    ...
    create_cpu_queue_prioritization_playbook,
...
_PLAYBOOK_INTENDED_UNCLEAN_EXITS = {
    ...
    "test_agent_crash": [
        "fboss_sw_agent",
        "fboss_hw_agent@0",
    ],  # both SIGKILLed by pkill -f fboss_
    "test_qspf_service_crash": ["qsfp_service"],
    "test_fsdb_crash": ["fsdb"],
}
...
# playbook. Cross-port probe: the downlink port's device group (advertising
# ``2001:db8:1::/64``) exchanges IPv6 traffic with the uplink port's
# (advertising ``2001:db8:2::/64``), so the DUT forwards every packet
# downlink<->uplink. See ``_build_loss_probe_traffic_item`` for the layout.
...
#       a live smoke disproved that assumption for this wedge800 build. Measure
...
class _LossCheckSpec(t.TypedDict):
    """Per-playbook IXIA_PACKET_LOSS_CHECK tuning (see the comment above)."""

    clear: bool
    sleep: int


_PLAYBOOK_LOSS_CHECK_SPEC: t.Dict[str, _LossCheckSpec] = {
...
def _annotate_check(
...
) -> t.Optional[PointInTimeHealthCheck]:
    ...
    * ``BGP_SESSION_ESTABLISH_CHECK`` — re-point ``ignore_all_prefixes_except``
      at our IXIA peers (downlink 2001:db8:3::2..::5 and uplink
      2001:db8:4::2..::5), pin the expected session count, and widen the
      retry budget (10 × 15s flat) so BGP has time to reconverge after an
      agent restart.
    * ``IXIA_PACKET_LOSS_CHECK`` — replace the entire ``input_json`` so the
      threshold points at our cross-port loss probe with the per-playbook
      ``clear_traffic_stats`` / ``sleep_time`` from ``_PLAYBOOK_LOSS_CHECK_SPEC``.
      Returns ``None`` (drop the check) for playbooks with no spec entry.

    ``jq_params`` is preserved in every branch. All other checks return
    unchanged. A ``None`` return means "drop this check for this playbook".
    """
    ...
    if check.name == hc_types.CheckName.SYSTEMCTL_ACTIVE_STATE_CHECK:
        ...
        if expected:
            return _override_check_params(
                check, {"expected_restarted_services": expected}
            )
        return check
    if check.name == hc_types.CheckName.UNCLEAN_EXIT_CHECK:
        ...
        if expected_unclean:
            return _override_check_params(check, {"exclude_services": expected_unclean})
        return check
    ...
    if check.name == hc_types.CheckName.IXIA_PACKET_LOSS_CHECK:
        ...
        # points at our cross-port loss probe instead of the factory's
        ...
        if spec is None:
            # No spec => this playbook doesn't touch the datapath, so there is
            # nothing meaningful to measure. Drop the check rather than leave
            # it: its factory ``input_json`` names the factory traffic items,
            # which this config replaces with the loss probe + RAW punt items,
            # so it would evaluate against names absent from the session.
            return None
...
def _annotate_checks(
    checks, playbook_name: str, dut_name: str
) -> t.List[PointInTimeHealthCheck]:
    """Filter out ``_SKIP_CHECKS`` and route the survivors through ``_annotate_check``.

    Checks ``_annotate_check`` returns ``None`` for are dropped as well.
    """
    annotated = [
        _annotate_check(c, playbook_name, dut_name)
        for c in (checks or [])
        if c.name not in _SKIP_CHECKS
    ]
    return [c for c in annotated if c is not None]
...
_CPU_PUNT_SUITE = [
    ...
    #
    # KNOWN FAILURE on wedge800: this ASIC traps hop-limit-0 to the
...
def _build_cpu_punt_playbooks(min_pps_overrides=None):
    """One playbook per RAW punt item, from the shared CPU-punt factories.
    ...
    playbooks = [
        create_cpu_punt_playbook(
            name=pb_name,
            traffic_item_name=item_name,
            active_queues=active_queues,
            no_discard_queues=no_discard_queues,
            min_out_pps=overrides.get(pb_name, default_min_pps),
        )
    ...
    # (doesn't fit the single-item suite); reuses the suite's traffic items.
    playbooks.append(
        create_cpu_queue_prioritization_playbook(
            traffic_item_names=[
                "TEST_RAW_TTL_1_IPV4_TRAFFIC",
...
def gen_rsw_test_config(
    *,
    ...
    loss_probe_name = _LOSS_PROBE_TRAFFIC_NAME_TMPL.format(host_upper=dut_name.upper())
    return tc(
        ...
        # _annotate_check (which drops a check by returning None). Then append
        # the CPU-punt suite (each of those starts only its own RAW item).
        playbooks=[
            pb(
                traffic_items_to_start=[loss_probe_name],
                prechecks=_annotate_checks(pb.prechecks, pb.name, dut_name),
                postchecks=_annotate_checks(pb.postchecks, pb.name, dut_name),
            )
