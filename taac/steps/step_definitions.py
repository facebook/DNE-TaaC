...
if not TAAC_OSS:
    ...
    from taac.utils.oss_driver_utils import wait_for_ping_reachable
...
if not TAAC_OSS:
    ...
    def run_bmc_cmd_hwcontrol(hostname: str, cmd: str) -> str:  # type: ignore
        ...
        with ParamikoClient(bmc_hostname, username=bmc_user, password=bmc_password) as client:
...
class SystemRebootStep(StepBase[taac_types.SystemRebootInput]):
    ...
    async def run(
        ...
        if input.trigger == taac_types.SystemRebootTrigger.FULL_SYSTEM_REBOOT:
            ...
            except (ConnectionError, EOFError, OSError, paramiko.SSHException, RuntimeError) as e:
                self.logger.info(
        ...
        await convert_to_async(wait_for_ping_reachable, ssh_entity=self.device.name, use_ipv6=use_ipv6)
        self.logger.info(
