...
RAW_URL = (
    "https://raw.githubusercontent.com/facebook/fbthrift/{rev}"
...
def sections_of(manifest: configparser.ConfigParser, family: str) -> List[str]:
    ...
    """
    return [s for s in manifest.sections() if s == family or s.startswith(family + ".")]


def value_in(
    manifest: configparser.ConfigParser, family: str, key: str
) -> Optional[str]:
...
def snapshot_path(overlay_dir: str) -> str:
    ...
    return os.path.join(
        os.path.dirname(os.path.abspath(overlay_dir)), "FBCODE_SNAPSHOT"
    )
...
def write_overlay(
    name: str, tag: str, sha: str, body: str, overlay_dir: str, dry_run: bool
) -> str:
...
def mode_check(args: argparse.Namespace, deps: Dict[str, Dep]) -> int:
    ...
    print(
        "PASS: every git dependency resolves to a fixed commit; the rest "
...
def resolve_for_write(
    deps: Dict[str, Dep], tag: str, body_rev: str, allow: set
) -> Tuple[List[Tuple[Dep, str, str]], List[Dep], List[Dep]]:
    """Split the graph into (resolved, missing-tag, unpinnable) for --write.

    Resolves everything before any overlay is written, so a missing tag cannot
    leave the overlays half-migrated.
    """
    ...
    for dep in sorted(deps.values()):
        ...
        if body is None:
            ...
            print(
                f"  {dep.name:22} SKIP      hand-maintained (no upstream "
                f"manifest){note}"
            )
    ...
    return resolved, skipped, unpinnable
...
def mode_write(args: argparse.Namespace, deps: Dict[str, Dep]) -> int:
    ...
    if fbthrift is None or fbthrift.url is None:
        die(
            "fbthrift-python is not in the dependency graph; cannot determine "
    ...
    resolved, skipped, unpinnable = resolve_for_write(deps, tag, body_rev, allow)

    if unpinnable:
        die(
            f"{plural(len(unpinnable))} floating with no upstream manifest "
    ...
    if skipped and not args.allow_partial:
        die(
            f"{plural(len(skipped))} lack the {tag} tag, so this snapshot "
    ...
    for dep, sha, body in resolved:
        action = write_overlay(dep.name, tag, sha, body, args.overlay_dir, args.dry_run)
        print(f"  {dep.name:22} {action:9} {sha[:12]}")

    if not args.dry_run:
        with open(snapshot_path(args.overlay_dir), "w") as fh:
            fh.write(
                "# GENERATED FILE -- do not hand-edit. The fbcode snapshot "
...
def main() -> int:
    ...
    action.add_argument(
        "--write", metavar="TAG", help="pin git dependencies to weekly tag TAG"
    )
    parser.add_argument(
        "--roots",
        nargs="+",
        default=DEFAULT_ROOTS,
        help="getdeps build targets to walk from (default: %(default)s)",
    )
    parser.add_argument(
        "--allow-floating",
        nargs="+",
        default=DEFAULT_ALLOW_FLOATING,
        help="manifests permitted to track HEAD (default: %(default)s)",
    )
    parser.add_argument(
        "--overlay-dir",
        default=OVERLAY_DIR,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="--write: report changes without writing"
    )
